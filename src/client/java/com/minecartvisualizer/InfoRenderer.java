...
public class InfoRenderer {
    ...
    public static void renderTexts(List<MutableText> infoTexts, Entity entity, MatrixStack matrices, VertexConsumerProvider vertexConsumerProvider) {
        ...
        renderTextLayer(infoTexts, textRenderer, matrix4f, vertexConsumerProvider, startY, true);
        renderTextLayer(infoTexts, textRenderer, matrix4f, vertexConsumerProvider, startY, false);

        matrices.pop();
    }

    private static void renderTextLayer(List<MutableText> texts, TextRenderer renderer, Matrix4f matrix, VertexConsumerProvider vertexConsumerProvider, float y, boolean isBackground) {
        float currentY = y;
        for (MutableText text : texts) {
            float x = -renderer.getWidth(text) / 2f;
            if (isBackground) {
                renderer.draw(text, x, currentY, -2130706433, false, matrix, vertexConsumerProvider, TextRenderer.TextLayerType.SEE_THROUGH, 0x4CC8C8C8, 0xF000F0);
            } else {
                renderer.draw(text, x, currentY, 0xFFFFFFFF, false, matrix, vertexConsumerProvider, TextRenderer.TextLayerType.NORMAL, 0, 0xF000F0);
            }
            currentY += 10;
        }
    }
    ...
    public static void renderHopperRanges(Entity entity, double cameraX, double cameraY, double cameraZ,
        ...
        WorldRenderer.drawBox(matrices, lines, viewPickupBox, 1.0f, 1.0f, 0.1f, 0.8f);
        ...
        WorldRenderer.drawBox(matrices, lines, viewExtractionBox, 1.0f, 1.0f, 0.1f, 0.8f);
    }
    ...
    public static boolean highlightExtractionTargets(Entity entity, double cameraX, double cameraY, double cameraZ,
        ...
        if (world.getBlockEntity(targetPos) != null) {
            ...
            if (!shape.isEmpty()) {
                ...
                for (Box box : shape.getBoundingBoxes()) {
                    ...
                    WorldRenderer.drawBox(matrices, lines, viewBox.expand(0.005), 0.0f, 1.0f, 0.0f, 1.0f);
                }
            }
        }
        ...
        if (!inventories.isEmpty()) {
            ...
            for (Entity invEntity : inventories) {
                ...
                WorldRenderer.drawBox(matrices, lines, viewBox.expand(0.01), 0.0f, 1.0f, 0.0f, 1.0f);
            }
        }
    ...
    public static void drawTrackerPointBox(MatrixStack matrices, VertexConsumer lines, TrackerColor color,
        ...
        WorldRenderer.drawBox(matrices, lines, standardBox.expand(0.005), r, g, b, a);
    }
}
